...
class SVRModel:
    def __init__(self, config, horizon: int, file_path: list, grid_search: bool):
        ...
        if grid_search:
            self.best_params, self.cv_results, self.param_grid, self.best_score = self.grid_search(
                train_from_date=train_from_date, test_from_date=test_from_date, kernel=kernel, C=C, degree=degree,
                gamma=gamma)
        else:
    ...
    def fit_and_predict(self, train_from_date, test_from_date, horizon, best_params, file_path):
        start = datetime.now()
        print("SVR fit and predict, train_from_date={}, test_from_date={}, horizon={}, best_params={}".format(
            train_from_date, test_from_date, horizon,
            best_params))
        x_train, x_test, y_train, y_test = self.preparator.get_scaled_data(test_from_date=test_from_date,
                                                                           train_from_date=train_from_date)
        model = SVR(kernel=best_params["kernel"], C=best_params["C"], degree=best_params["degree"], gamma=best_params["gamma"], max_iter=-1, shrinking=True, tol=0.001)  # verbose=True
    ...
    def multistep_forecast(self, train_from_date, test_from_date, test_to_date, horizon, best_params, file_path):
        start = datetime.now()
